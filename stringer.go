...
func main() {
	...
	if *proto {
		pg.parsePackage(args)
		if *protoPkg == "" {
			*protoPkg = pg.pkg.name
		}
...
`
		...
		pg.Printf(header,  strings.Join(os.Args[1:], " "), *protoPkg, *protoGoPkg)
		// Run generate for each type.
		for _, typeName := range types {
			pg.generate(typeName, false, false, false, false, *transformMethod, *trimPrefix, *lineComment, true)
		}
		// Format the output.
		src := pg.format()
	...
	// Run generate for each type.
	for _, typeName := range types {
		g.generate(typeName, *json, *yaml, *sql, *text, *transformMethod, *trimPrefix, *lineComment, false)
	}
...
func (g *Generator) generate(typeName string, includeJSON, includeYAML, includeSQL, includeText bool, transformMethod string, trimPrefix string, lineComment bool, protoOnly bool) {
